...
> **测试环境**：阿里云云主机 · 8 vCPU / 16G 内存 / NVMe SSD · Ubuntu 22.04 · 多节点进程同机（本地环回）· ghz（gRPC）/ wrk（HTTP）· 每项 10s。
...
> ① 数据为 2026-08-20 阿里云实测（8 vCPU / 16G / NVMe SSD / Ubuntu 22.04，多节点进程同机环回），为**审查加固后（commit e5d737b 加锁）当前代码**实测；加锁前后同环境对比差异全部在 ±5% 内（详见 [docs/性能压测报告.md](docs/性能压测报告.md) §6）。
